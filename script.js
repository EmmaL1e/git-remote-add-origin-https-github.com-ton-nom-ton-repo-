document.addEventListener('DOMContentLoaded', () => {
    // Embedded games data to bypass browser CORS (file:///) restrictions when launching index.html locally
    const QUIZ_GAMES = {
        1: {
            "questions": [
                {
                ...
                {
                    "métier": "boulangerie",
                    "question": "Que signifie la mention DLC sur vos affichages en magasin ?",
                    "réponses": [
            ...
            "questions": [
                ...
                },
                {
                    ...
                    "réponses": [
                        ...
                        "Identifier la différence jusqu’à l’inventaire",
    ...
    // Initialize Game Index from LocalStorage and sanitize it strictly
    function initGameIndex() {
        let savedIndex = localStorage.getItem('carrefourQuizGameIndex');
        let parsed = parseInt(savedIndex, 10);
        if (isNaN(parsed) || parsed < 1 || parsed > 5) {
            parsed = 1;
            localStorage.setItem('carrefourQuizGameIndex', '1');
        }
        quizData.gameIndex = parsed;
    }
    ...
    function finishQuiz() {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem('carrefourQuizGameIndex', String(nextIndex));
        }, 500);
    }
    ...
    // Subtle parallax effect on mouse move for that premium feel
    document.addEventListener('mousemove', (e) => {
        const mouseX = e.clientX / window.innerWidth - 0.5;
        const mouseY = e.clientY / window.innerHeight - 0.5;

        const activeContent = document.querySelector('.screen.active .content-wrapper');
        if (activeContent && window.innerWidth > 1024) {
            activeContent.style.transform = `perspective(1000px) rotateY(${mouseX * 5}deg) rotateX(${-mouseY * 5}deg)`;
        }
    });
});
